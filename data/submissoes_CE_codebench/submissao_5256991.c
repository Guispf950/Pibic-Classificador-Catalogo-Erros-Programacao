...
void node_avl_update_height(NodeAVL *root){
  if(root==NULL) return;
...
NodeAVL* node_avl_rotate_right(NodeAVL *x){
  ...
  x->left = y->right;
  y->right = x;
  node_avl_update_height(x);
  node_avl_update_height(y);
  return y;
}
...
void question_print(NodeAVL *root){
...
int main(){
  ...
  do{
    ...
    if(exist) question_print(root);
  }while(x!=0);

  node_avl_free_all(root); root = NULL;
